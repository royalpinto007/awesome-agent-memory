...
<!-- LIST:START -->
**42 entries**, auto-refreshed weekly. Star counts updated **2026-08-10**. Browse the filterable version at **[agent-memory.agentpostmortem.com](https://agent-memory.agentpostmortem.com)**.

### Memory frameworks and libraries

- [Mem0](https://github.com/mem0ai/mem0) `* 62.9k`: Self-editing memory layer with user/session/agent scopes over a hybrid vector-plus-graph-plus-KV store; strong LoCoMo/LongMemEval scores.
- [Cognee](https://github.com/topoteretes/cognee) `* 29.9k`: Turns ingested data into a reasoning knowledge graph so memory improves from corrections; graph-first.
- [Graphiti](https://github.com/getzep/graphiti) `* 29.7k`: Zep's framework for building real-time, temporally-aware knowledge graphs for agent memory.
- [Letta (formerly MemGPT)](https://github.com/letta-ai/letta) `* 24.2k`: OS-inspired agent runtime where the agent self-manages tiered memory (main context as RAM, archival as disk) via memory tools and a REST API.
...
- [A-Mem](https://github.com/WujiangXu/A-mem-sys) `* 381`: Zettelkasten-inspired agentic memory that links structured knowledge notes for dynamic consolidation.
...
- [LangChain / LangGraph Memory](https://github.com/langchain-ai/langchain) `* 143.9k`: Widely used agent framework providing conversation buffers, summary memory, and state persistence/checkpointing.
- [LlamaIndex Memory](https://github.com/run-llama/llama_index) `* 51.5k`: Data framework with built-in chat memory buffers, vector memory, and composable memory modules for agents.
- [GraphRAG (Microsoft)](https://github.com/microsoft/graphrag) `* 35.4k`: Graph-based RAG that builds a knowledge graph from documents for structured, memory-like retrieval.
...
- [Milvus](https://github.com/milvus-io/milvus) `* 45.6k`: Scalable open-source vector database for storing and retrieving embedding-based memories.
...
- [Qdrant](https://github.com/qdrant/qdrant) `* 33.9k`: High-performance open-source vector database commonly used as the embedding store for agent long-term memory.
- [Chroma](https://github.com/chroma-core/chroma) `* 29k`: Developer-friendly embedded vector database popular for prototyping agent memory.
...
- [FalkorDB](https://github.com/FalkorDB/FalkorDB) `* 5.5k`: Low-latency graph database designed for GraphRAG and agent knowledge-graph memory.
...
- [LongMemEval](https://github.com/xiaowu0162/LongMemEval) `* 989`: 500-question benchmark for chat-assistant long-term interactive memory across six abilities.
...
- [Awesome-Memory-for-Agents (papers)](https://github.com/TsinghuaC3I/Awesome-Memory-for-Agents) `* 630`: Curated academic collection of papers on memory for language agents.
...
- [Agent Memory Techniques (notebooks)](https://github.com/NirDiamant/Agent_Memory_Techniques) `* 853`: 30 runnable notebooks: buffers, vector stores, KGs, episodic/semantic memory, MemGPT, Mem0, Letta, Zep, Graphiti, LoCoMo.
